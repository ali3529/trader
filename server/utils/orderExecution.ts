...
import { privateRequest, loadKeys, assertValidSymbol } from "./nobitex";
import { hasUnresolvedOrder, loadOrderLedger, recordBotOrder } from "./orderLedger";
...
/** صف سریال سفارش‌های واقعی — هرگز دو سفارش هم‌زمان ارسال نمی‌شود */
export function submitRealOrder(body: OrderBody): Promise<OrderExecutionResult> {
  const operation = orderQueue.then(() => submitOrder(body)) as Promise<OrderExecutionResult>;
  orderQueue = operation.catch(() => undefined);
  return operation;
}

async function submitOrder(body: OrderBody): Promise<OrderExecutionResult> {
  // گیت اول باید mode سراسری Real باشد. این بررسی داخل صف و درست پیش از اجرا
  // انجام می‌شود تا تغییر هم‌زمان حالت به Paper نیز سفارش را fail-closed کند.
  if ((await getBotMode()) !== "real") {
    throw createError({ statusCode: 403, statusMessage: "ثبت سفارش فقط در حالت Real مجاز است" });
  }
  const requestedSymbol = String(body?.symbol ?? "").toUpperCase().replace(/IRR$/, "IRT");
  if (hasUnresolvedOrder(await loadOrderLedger(), requestedSymbol)) {
    throw createError({
      statusCode: 409,
      statusMessage: "برای این نماد سفارش مبهم قبلی وجود دارد؛ ابتدا وضعیت آن را در صرافی بررسی کنید",
    });
  }
  ...
  const response: OrderExecutionResult = {
    ...
    clientOrderId: String(confirmed.clientOrderId ?? clientOrderId),
    orderStatus: String(confirmed.status ?? result.order.status ?? "Unknown"),
    // در وضعیت مبوم، دفتر داخلی مجاز نیست بر اساس مقدار احتمالی تغییر کند.
 ...
 */
async function submitRamzinexOrder(body: OrderBody): Promise<OrderExecutionResult> {
  ...
  // سفارش از این نقطه واقعاً ایجاد شده است؛ حتی اگر استعلام وضعیت شکست بخورد باید
  // یک رکورد reconciliation ذخیره شود تا tick بعدی سفارش را دوباره ارسال نکند.
  let confirmed: RzOrder = {
    id: orderId,
    pairId: 0,
    symbol,
    side: type,
    price: estimatedPriceToman * 10,
    qty: quantity,
    filled: 0,
    statusId: 1,
    status: "unknown",
  ...
  try {
    ...
    // سفارش باز یا partially-filled باید لغو و دوباره خوانده شود تا مقدار fill
    // نهایی و ماندهٔ باز قطعی شود.
    if (confirmed.statusId === 1 || confirmed.statusId === 4) {
  ...
  const response: OrderExecutionResult = {
    ...
    orderId,
    clientOrderId,
    orderStatus: String(confirmed.status ?? "Unknown"),
